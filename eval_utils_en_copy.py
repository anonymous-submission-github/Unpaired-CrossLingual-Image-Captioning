...
def language_eval(dataset, preds, model_id, split):
    ...
    annFile='data/aic_process/ALL_11683_v3_COCOCN_val_test_en.json'
...
def eval_split(model, crit, loader, p_flag, eval_kwargs={}):
    ...
    while True:
        ...
        if data.get('labels', None) is not None and verbose_loss:
            ...
            if use_rela == 1:
                ...
                if  p_flag == 1:
                    tmp = [data['fc_feats'], data['ssg_paired_rela_matrix'], data['ssg_paired_rela_masks'],
                           data['ssg_paired_obj'], data['ssg_paired_obj_masks'],
            ...
            with torch.no_grad():
                ...
                outputs, _, _, _ = model(fc_feats, labels, rela_data, ssg_data)
        ...
        if use_ssg == 1:
            if p_flag== 0:
                ...
                tmp = [data['fc_feats'][np.arange(loader.batch_size) * loader.seq_per_img],
                       data['ssg_paired_rela_matrix'][np.arange(loader.batch_size) * loader.seq_per_img],
                       data['ssg_paired_rela_masks'][np.arange(loader.batch_size) * loader.seq_per_img],
                       data['ssg_paired_obj'][np.arange(loader.batch_size) * loader.seq_per_img],
                       data['ssg_paired_obj_masks'][np.arange(loader.batch_size) * loader.seq_per_img],
                       data['ssg_paired_attr'][np.arange(loader.batch_size) * loader.seq_per_img],
                       data['ssg_paired_attr_masks'][np.arange(loader.batch_size) * loader.seq_per_img],
                       data['labels_p'][np.arange(loader.batch_size) * loader.seq_per_img],
                       data['masks_p'][np.arange(loader.batch_size) * loader.seq_per_img]]
        ...
        if p_flag==0:
            ...
            if beam_size > 1 and verbose_beam:
                for i in range(loader.batch_size):
                    print('\n'.join([utils.decode_sequence(loader.get_vocab_p(), _['seq'].unsqueeze(0),
                                                           use_ssg=use_ssg)[0] for _ in model.done_beams[i]]))
                    print('--' * 10)
            sents = utils.decode_sequence(loader.get_vocab_p(), seq, use_ssg=use_ssg)
            gt_captions = utils.decode_sequence(loader.get_vocab_p(), labels[:,1:], use_ssg=use_ssg)

            for k, sent in enumerate(sents):
                entry = {'image_id': data['infos_p'][k]['id'], 'caption': sent}
                entry['gt_caption'] = gt_captions[k]
                if eval_kwargs.get('dump_path', 1) == 1:
                    entry['file_name'] = data['infos_p'][k]['file_path']

                if eval_kwargs.get('dump_impages', 0) == 1:
                    # dump the raw image to vis/ folder
                    cmd = 'cp "' + os.path.join(eval_kwargs['image_root'], data['infos_p'][k]['file_path']) + '" vis/imgs/img' + str(len(predictions)) + '.jpg' # bit gross
                    print(cmd)
                    os.system(cmd)
                predictions.append(entry)
    ...
    if lang_eval == 1:
        if p_flag==0:
            ...
            lang_stats, scores_each = language_eval(dataset, predictions, eval_kwargs['id_p'], split)
            if verbose:
                for img_id in scores_each.keys():
                    print('image %s, %s' % (scores_each[img_id]['image_id'], scores_each[img_id]['caption']))
                    print('cider {0:2f}'.format(scores_each[img_id]['CIDEr']))
                    text_file = open('gen_cap/cap_' + eval_kwargs['id_p'] + '.txt', "aw")
                    text_file.write('image %s, %s' % (scores_each[img_id]['image_id'], scores_each[img_id]['caption']))
                    text_file.write('\n cider {0:2f}'.format(scores_each[img_id]['CIDEr']))
                    text_file.write('\n')
                    text_file.close()
