...
def clean_short_version(sent):
    ...
    sent = re.sub(r"this's", "this is", sent)
...
##—————————————————————————————————————————— read json start
  ...
  cocobu_cococn = json.load(open(
    'data/coco_cn/cocobu_gan_isg.json'))
  ...
  # for i in imgs.keys():
    ...
    temp =clean_short_version(imgs[i].decode('utf-8'))
    temp = cleanData(temp)
    # when caption stored in list
    # temp = cleanData(imgs[i][0].decode('utf-8'))
    li = list(filter(lambda x: len(x) > 0, temp.split(" ")))
    # li=tokenize(temp)
    tmp['sentids']=[]
...
if __name__ == "__main__":
  ...
  parser.add_argument('--input_path', default='data/aic_process/ALL_11683_COCOCN_en_v3.json', help='input wmt data file to process into hdf5')
  parser.add_argument('--output_json', default='data/aic_process/cocobu_ALL_11683_v3_COCOCN_t5_en.json', help='output json file')
  parser.add_argument('--output_h5', default='data/aic_process/cocobu_ALL_11683_v3_COCOCN_t5_en', help='output h5 file')
