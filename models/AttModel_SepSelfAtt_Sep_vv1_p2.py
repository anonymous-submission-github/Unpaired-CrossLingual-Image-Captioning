...
class AttModel(CaptionModel):
    ...
    def _forward(self, fc_feats, seq, isg_data=None, ssg_data=None):
        ...
        # att = self.mapping_triplet_dim(torch.cat([obj_feats, rela_feats, attr_feats], 1))
        # att = self.map_dim(att)
        ...
        for i in range(seq.size(1) - 1):
            ...
            if i >= 1 and seq[:, i].sum() == 0:
                break
            output, state, _ = self.get_logprobs_state(it, fc_feats, obj_feats, rela_feats, attr_feats, state)
            outputs[:, i] = output
    ...
    def get_logprobs_state(self, it, fc_feats, obj_feats, rela_feats, attr_feats, state):
        ...
        output, state, att = self.core(xt, fc_feats, obj_feats, rela_feats, state, attr_feats)
    ...
    def _sample_beam(self, fc_feats, isg_data=None, ssg_data=None, opt={}):
        ...
        for k in range(batch_size):
            ...
            for t in range(1):
                ...
                logprobs, state,_ = self.get_logprobs_state(it, tmp_fc_feats, tmp_obj_feats, tmp_rela_feats, tmp_attr_feats, state)
...
class GTssgCore_SepSelfAtt_SEP_vv1_p2(nn.Module):
    def __init__(self, opt, use_maxout=False):
        super(GTssgCore_SepSelfAtt_SEP_vv1_p2, self).__init__()
        self.drop_prob_lm = opt.drop_prob_lm
    ...
    def forward(self, xt, fc_feats, obj_feats, rela_feats, state, attr_feats=None):
        ...
        # state[0] means hidden state and state[1] means cell,
        # state[0][i] means the i-th layer's hidden state
...
class GTssgModel_SepSelfAtt_SEP_vv1_p2(AttModel):
    def __init__(self, opt):
        super(GTssgModel_SepSelfAtt_SEP_vv1_p2, self).__init__(opt)
        self.num_layers = 2
        self.core = GTssgCore_SepSelfAtt_SEP_vv1_p2(opt)
