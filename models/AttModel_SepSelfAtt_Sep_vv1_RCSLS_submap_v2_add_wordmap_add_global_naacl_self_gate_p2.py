...
class AttModel(CaptionModel):
    ...
    def ssg_gfc(self, ssg_data):
        ...
        ssg_obj_feats_w = torch.zeros([ssg_obj.size()[0], ssg_obj.size()[1], self.rnn_size]).cuda()
        ssg_obj_feats_s = torch.zeros([ssg_obj.size()[0], ssg_obj.size()[1], self.rnn_size]).cuda()
        ssg_obj_feats_g = torch.zeros([ssg_obj.size()[0], ssg_obj.size()[1], self.rnn_size]).cuda()
        ...
        for img_id in range(N_img):
            ...
            ssg_obj_feats_w[img_id * seq_per_img: (img_id + 1) * seq_per_img, :N_obj, :] = obj_feats_temp_wl
            ...
            ssg_obj_feats_s[img_id * seq_per_img: (img_id + 1) * seq_per_img, :N_obj, :] = obj_feats_temp_zh
            ...
            ssg_obj_feats_g[img_id * seq_per_img: (img_id + 1) * seq_per_img, :N_obj, :] = obj_feat_temp_gl
        ...
        ssg_data_new['ssg_obj_feats_w'] = ssg_obj_feats_w
        ssg_data_new['ssg_obj_feats_s'] = ssg_obj_feats_s
        ssg_data_new['ssg_obj_feats_g'] = ssg_obj_feats_g
    ...
    def merge_ssg_att(self, ssg_data_new):
        ...
        """
        ssg_obj_feats_w = ssg_data_new['ssg_obj_feats_w']
        ssg_obj_feats_s = ssg_data_new['ssg_obj_feats_s']
        ssg_obj_feats_g = ssg_data_new['ssg_obj_feats_g']
        ...
        ssg_obj_size = ssg_obj_feats_w.size()
        N_att = ssg_obj_size[0]
        if self.index_eval == 1:
            seq_per_img = 1
        else:
        ...
        if N_obj != 0:
            obj_feats_w = self.self_att_obj_w(ssg_obj_feats_w)
            obj_feats_s = self.self_att_obj_s(ssg_obj_feats_s)
            obj_feats_g = self.self_att_obj_g(ssg_obj_feats_g)
        else:
            obj_feats_w = torch.zeros([ssg_obj_feats_w.shape[0], self.rnn_size]).cuda()
            obj_feats_s = torch.zeros([ssg_obj_feats_s.shape[0], self.rnn_size]).cuda()
            obj_feats_g = torch.zeros([ssg_obj_feats_g.shape[0], self.rnn_size]).cuda()
    ...
    def _forward(self,  fc_feats, seq, isg_data=None, ssg_data=None):
        ...
        if self.use_ssg:
            fc_feats = self.fc_embed(fc_feats)
            ssg_data_new = self.ssg_gfc(ssg_data)
            obj_feats_w, obj_feats_s, obj_feats_g, rela_feats, attr_feats = self.merge_ssg_att(ssg_data_new)
            ## concat word level and subgraph level info to get object embeding
            # obj_feats = self.mapping_hierachy(torch.cat([obj_feats_w, obj_feats_s, obj_feats_g],1))
            weight=self.mlp(obj_feats_w)
            obj_feats = (torch.cat([obj_feats_w.unsqueeze(-1), obj_feats_s.unsqueeze(-1), obj_feats_g.unsqueeze(-1)],-1)* weight.unsqueeze(1)).sum(dim=-1)

        att_obj = self.map_dim_obj(obj_feats)
        att_rela = self.map_dim_rela(rela_feats)
        att_attr = self.map_dim_attr(attr_feats)
        for i in range(seq.size(1) - 1):
        ...
        return outputs,att_obj,att_rela,att_attr
...
class GTssgCore_SepSelfAtt_SEP_vv1_RCSLS_submap_v2_add_wordmap_add_global_naacl_self_gate_p2(nn.Module):
    def __init__(self, opt, use_maxout=False):
        super(GTssgCore_SepSelfAtt_SEP_vv1_RCSLS_submap_v2_add_wordmap_add_global_naacl_self_gate_p2, self).__init__()
        self.drop_prob_lm = opt.drop_prob_lm
...
class GTssgModel_SepSelfAtt_SEP_vv1_RCSLS_submap_v2_add_wordmap_add_global_naacl_self_gate_p2(AttModel):
    def __init__(self, opt):
        super(GTssgModel_SepSelfAtt_SEP_vv1_RCSLS_submap_v2_add_wordmap_add_global_naacl_self_gate_p2, self).__init__(opt)
        self.num_layers = 2
        self.core = GTssgCore_SepSelfAtt_SEP_vv1_RCSLS_submap_v2_add_wordmap_add_global_naacl_self_gate_p2(opt)
