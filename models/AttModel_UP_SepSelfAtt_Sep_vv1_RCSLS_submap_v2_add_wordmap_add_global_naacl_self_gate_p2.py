...
class AttModel(CaptionModel):
    def __init__(self, opt):
        ...
        self.map_dim=nn.Linear(1000,100)
        self.map_dim_obj=nn.Linear(1000,100)
        self.map_dim_attr=nn.Linear(1000,100)
        self.map_dim_rela=nn.Linear(1000,100)
        self.mapping_triplet_dim = nn.Sequential(nn.Linear(opt.rnn_size * 3, opt.rnn_size),
        ...
        # self.graph_weights = nn.Parameter(torch.ones(3,1))
        # self.graph_weights_double = nn.Parameter(torch.ones(2,1))
        self.mlp = MLP(opt)
        # self.self_att_weight = SelfAttention_weight(opt)
        ...
        self.self_att_obj_w = SelfAttention(opt)
        self.self_att_obj_s = SelfAttention(opt)
        self.self_att_obj_g = SelfAttention(opt)
    ...
    def sg_gfc(self, sg_data, type):
        ...
        sg_obj_feats_w = torch.zeros([sg_obj.size()[0], sg_obj.size()[1], self.rnn_size]).cuda()
        sg_obj_feats_s = torch.zeros([sg_obj.size()[0], sg_obj.size()[1], self.rnn_size]).cuda()
        sg_obj_feats_g = torch.zeros([sg_obj.size()[0], sg_obj.size()[1], self.rnn_size]).cuda()
        ...
        for img_id in range(N_img):
            ...
            obj_feats_ori_wl = self.get_RCSLS_embed_word_level(sg_obj[img_id * seq_per_img, :N_obj])
            obj_feats_temp_wl = self.obj_obj_fc(obj_feats_ori_wl)

            ## get global level mapping
            obj_feats_ori = self.get_RCSLS_en_embed(sg_obj[img_id * seq_per_img, :N_obj])
            ...
            ## get word level for obj feature
            obj_num = np.ones([N_obj, ])
            ...
            for rela_id in range(N_rela):
                ...
                obj_feats_temp_wl[sbj_id] = obj_feats_temp_wl[sbj_id] + self.sbj_rela_fc(torch.cat((sbj_feat_zh, obj_feat_zh, rela_feat_zh)))
                obj_num[sbj_id] = obj_num[sbj_id] + 1.0
                obj_feats_temp_wl[obj_id] = obj_feats_temp_wl[obj_id] + self.obj_rela_fc(torch.cat((sbj_feat_zh, obj_feat_zh, rela_feat_zh)))
                obj_num[obj_id] = obj_num[obj_id] + 1.0
                rela_feats_temp_zh[rela_id] = self.rela_fc(torch.cat((sbj_feat_zh, obj_feat_zh, rela_feat_zh)))
            for obj_id in range(N_obj):
                obj_feats_temp_wl[obj_id] = obj_feats_temp_wl[obj_id] / obj_num[obj_id]
            ...
            for obj_id in range(N_obj):
                ...
                if N_attr != 0:
                    attr_feat_ori = self.get_RCSLS_en_embed(sg_attr[img_id * seq_per_img, obj_id, :N_attr])
                    attr_feat_ori = self.map_emb_dim(attr_feat_ori)
                    attr_feat_ori_zh = self.en2zh_maping(attr_feat_ori)
                    for attr_id in range(N_attr):
                        attr_feats_temp_zh[obj_attr_ids] = attr_feats_temp_zh[obj_attr_ids] + self.attr_fc(torch.cat((obj_feats_temp_wl[obj_id], attr_feat_ori_zh[attr_id])))
            ...
            sg_obj_feats_w[img_id * seq_per_img: (img_id + 1) * seq_per_img, :N_obj, :] = obj_feats_temp_wl

            ## get subgraph level for obj feature
            obj_num = np.ones([N_obj, ])
            rela_feats_temp_zh = torch.zeros([N_rela, self.rnn_size])
            for rela_id in range(N_rela):
                sbj_id = int(sg_rela_matrix[img_id * seq_per_img, rela_id, 0])
                ...
                rela_feat_zh = self.en2zh_maping(rela_feat)
                rela_feat_zh = rela_feat_zh.squeeze(0)
                obj_feats_temp_zh[sbj_id] = obj_feats_temp_zh[sbj_id] + self.sbj_rela_fc(torch.cat((sbj_feat_zh, obj_feat_zh, rela_feat_zh)))
                obj_num[sbj_id] = obj_num[sbj_id] + 1.0
                obj_feats_temp_zh[obj_id] = obj_feats_temp_zh[obj_id] + self.obj_rela_fc(torch.cat((sbj_feat_zh, obj_feat_zh, rela_feat_zh)))
                obj_num[obj_id] = obj_num[obj_id] + 1.0
                rela_feats_temp_zh[rela_id] = self.rela_fc(torch.cat((sbj_feat_zh, obj_feat_zh, rela_feat_zh)))
            for obj_id in range(N_obj):
                obj_feats_temp_zh[obj_id] = obj_feats_temp_zh[obj_id] / obj_num[obj_id]
            ...
            for obj_id in range(N_obj):
                ...
                if N_attr != 0:
                    ...
                    attr_feat_ori_zh = self.en2zh_maping(attr_feat_ori)
                    for attr_id in range(N_attr):
                        attr_feats_temp_zh[obj_attr_ids] = attr_feats_temp_zh[obj_attr_ids] + self.attr_fc(torch.cat((obj_feats_temp_zh[obj_id], attr_feat_ori_zh[attr_id])))
                    attr_feats_temp_zh[obj_attr_ids] = attr_feats_temp_zh[obj_attr_ids] / (N_attr + 0.0)
                    obj_attr_ids += 1
            N_obj_attr = obj_attr_ids
            sg_attr_masks_new[img_id * seq_per_img:(img_id + 1) * seq_per_img, :N_obj_attr] = 1

            sg_obj_feats_s[img_id * seq_per_img: (img_id + 1) * seq_per_img, :N_obj, :] = obj_feats_temp_zh

            ## get global level for obj feature
            obj_num = np.ones([N_obj, ])
            rela_feats_temp_zh = torch.zeros([N_rela, self.rnn_size])
            for rela_id in range(N_rela):
                sbj_id = int(sg_rela_matrix[img_id * seq_per_img, rela_id, 0])
                ...
                rela_feat_zh = self.en2zh_maping(rela_feat)
                rela_feat_zh = rela_feat_zh.squeeze(0)
                obj_feat_temp_gl[sbj_id] = obj_feat_temp_gl[sbj_id] + self.sbj_rela_fc(torch.cat((sbj_feat_zh, obj_feat_zh, rela_feat_zh)))
                obj_num[sbj_id] = obj_num[sbj_id] + 1.0
                obj_feat_temp_gl[obj_id] = obj_feat_temp_gl[obj_id] + self.obj_rela_fc(torch.cat((sbj_feat_zh, obj_feat_zh, rela_feat_zh)))
                obj_num[obj_id] = obj_num[obj_id] + 1.0
                rela_feats_temp_zh[rela_id] = self.rela_fc(torch.cat((sbj_feat_zh, obj_feat_zh, rela_feat_zh)))
            for obj_id in range(N_obj):
                obj_feat_temp_gl[obj_id] = obj_feat_temp_gl[obj_id] / obj_num[obj_id]
            ...
            for obj_id in range(N_obj):
                ...
                if N_attr != 0:
                    ...
                    attr_feat_ori_zh = self.en2zh_maping(attr_feat_ori)
                    for attr_id in range(N_attr):
                        attr_feats_temp_zh[obj_attr_ids] = attr_feats_temp_zh[obj_attr_ids] + self.attr_fc(torch.cat((obj_feat_temp_gl[obj_id], attr_feat_ori_zh[attr_id])))
                    attr_feats_temp_zh[obj_attr_ids] = attr_feats_temp_zh[obj_attr_ids] / (N_attr + 0.0)
                    obj_attr_ids += 1
            N_obj_attr = obj_attr_ids
            sg_attr_masks_new[img_id * seq_per_img:(img_id + 1) * seq_per_img, :N_obj_attr] = 1

            sg_obj_feats_g[img_id * seq_per_img: (img_id + 1) * seq_per_img, :N_obj, :] = obj_feat_temp_gl



            if N_rela != 0:
                sg_rela_feats[img_id * seq_per_img: (img_id + 1) * seq_per_img, :N_rela, :] = rela_feats_temp_zh
            if N_obj_attr != 0:
                sg_attr_feats[img_id * seq_per_img: (img_id + 1) * seq_per_img, :N_obj_attr, :] = attr_feats_temp_zh[:N_obj_attr]

        sg_data_new[type +'_obj_feats_w'] = sg_obj_feats_w
        sg_data_new[type +'_obj_feats_s'] = sg_obj_feats_s
        sg_data_new[type +'_obj_feats_g'] = sg_obj_feats_g
    ...
    def merge_sg_att(self, sg_data_new, type):
        """
        merge sg_obj_feats, sg_rela_feats, sg_attr_feats together
        :param sg_data_new:
        :return: att_feats: [N_att, N_att_max, rnn_size]
                 att_masks: [N_att, N_att_max]
        """
        sg_obj_feats_w = sg_data_new[type +'_obj_feats_w']
        sg_obj_feats_s = sg_data_new[type +'_obj_feats_s']
        sg_obj_feats_g = sg_data_new[type +'_obj_feats_g']
        ...
        ssg_obj_size = sg_obj_feats_w.size()
        ...
        if N_obj != 0:
            obj_feats_w = self.self_att_obj_w(sg_obj_feats_w)
            obj_feats_s = self.self_att_obj_s(sg_obj_feats_s)
            obj_feats_g = self.self_att_obj_g(sg_obj_feats_g)
        else:
            obj_feats_w = torch.zeros([sg_obj_feats_w.shape[0], self.rnn_size]).cuda()
            obj_feats_s = torch.zeros([sg_obj_feats_s.shape[0], self.rnn_size]).cuda()
            obj_feats_g = torch.zeros([sg_obj_feats_g.shape[0], self.rnn_size]).cuda()
        ...
        return obj_feats_w, obj_feats_s, obj_feats_g, rela_feats, attr_feats
    ...
    def _sample_beam(self, fc_feats, isg_data=None, ssg_data=None, opt={}):
        ...
        fc_feats = self.fc_embed(fc_feats)
        ssg_data_new = self.sg_gfc(ssg_data,'ssg')
        _obj_feats_w, _obj_feats_s, _obj_feats_g, _rela_feats, _attr_feats = self.merge_sg_att(ssg_data_new, 'ssg')
        ## concat word level and subgraph level info to get object embeding
        weight = self.mlp(_obj_feats_w)
        _obj_feats = (torch.cat([_obj_feats_w.unsqueeze(-1), _obj_feats_s.unsqueeze(-1), _obj_feats_g.unsqueeze(-1)], -1) * weight.unsqueeze(1)).sum(dim=-1)
...
class GTssgCore_UP_SepSelfAtt_SEP_vv1_RCSLS_submap_v2_add_wordmap_add_global_naacl_self_gate_p2(nn.Module):
    def __init__(self, opt, use_maxout=False):
        super(GTssgCore_UP_SepSelfAtt_SEP_vv1_RCSLS_submap_v2_add_wordmap_add_global_naacl_self_gate_p2, self).__init__()
        self.drop_prob_lm = opt.drop_prob_lm
...
class GTssgModel_UP_SepSelfAtt_SEP_vv1_RCSLS_submap_v2_add_wordmap_add_global_naacl_self_gate_p2(AttModel):
    def __init__(self, opt):
        super(GTssgModel_UP_SepSelfAtt_SEP_vv1_RCSLS_submap_v2_add_wordmap_add_global_naacl_self_gate_p2, self).__init__(opt)
        self.num_layers = 2
        self.core = GTssgCore_UP_SepSelfAtt_SEP_vv1_RCSLS_submap_v2_add_wordmap_add_global_naacl_self_gate_p2(opt)
