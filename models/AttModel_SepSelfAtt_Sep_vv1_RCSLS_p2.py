...
class AttModel(CaptionModel):
    ...
    # def get_RCSLS_embed(self, idx_tensor,model_zh,model_en,itow_zh,itow):
    def get_RCSLS_embed(self, idx_tensor):
        try:
    ...
    def ssg_gfc(self, ssg_data):
        ...
        for img_id in range(N_img):
            ...
            if N_obj == 0:
                continue
            # tmp1=ssg_obj[img_id * seq_per_img, :N_obj]
            # obj_feats_ori = self.get_RCSLS_embed(ssg_obj[img_id * seq_per_img, :N_obj],model_zh,model_en,itow_zh,itow)
            obj_feats_ori = self.get_RCSLS_embed(ssg_obj[img_id * seq_per_img, :N_obj])
            # obj_feats_ori_old = self.embed(ssg_obj[img_id * seq_per_img, :N_obj].cuda().long())
            ...
            for rela_id in range(N_rela):
                ...
                # tmp2=rela_index
                # rela_feat = self.get_RCSLS_embed(rela_index,model_zh,model_en,itow_zh,itow)
                rela_feat = self.get_RCSLS_embed(rela_index)
                rela_feat = rela_feat.squeeze(0)
                # rela_feat_old = self.embed(rela_index.cuda().long())
            ...
            for obj_id in range(N_obj):
                ...
                if N_attr != 0:
                    # tmp3=ssg_attr[img_id * seq_per_img, obj_id, :N_attr]
                    # attr_feat_ori = self.get_RCSLS_embed(ssg_attr[img_id * seq_per_img, obj_id, :N_attr],model_zh,model_en,itow_zh,itow)
                    attr_feat_ori = self.get_RCSLS_embed(ssg_attr[img_id * seq_per_img, obj_id, :N_attr])
                    # attr_feat_ori = self.embed(ssg_attr[img_id * seq_per_img, obj_id, :N_attr].cuda().long())
                    for attr_id in range(N_attr):
                        attr_feats_temp[obj_attr_ids] = attr_feats_temp[obj_attr_ids] + self.attr_fc(torch.cat((obj_feats_ori[obj_id], attr_feat_ori[attr_id])))
...
class GTssgCore_SepSelfAtt_SEP_vv1_RCSLS_p2(nn.Module):
    def __init__(self, opt, use_maxout=False):
        super(GTssgCore_SepSelfAtt_SEP_vv1_RCSLS_p2, self).__init__()
        self.drop_prob_lm = opt.drop_prob_lm
...
class GTssgModel_SepSelfAtt_SEP_vv1_RCSLS_p2(AttModel):
    def __init__(self, opt):
        super(GTssgModel_SepSelfAtt_SEP_vv1_RCSLS_p2, self).__init__(opt)
        self.num_layers = 2
        self.core = GTssgCore_SepSelfAtt_SEP_vv1_RCSLS_p2(opt)
