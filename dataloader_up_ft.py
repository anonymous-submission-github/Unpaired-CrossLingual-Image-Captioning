...
class DataLoader_UP_FT(data.Dataset):
    ...
    def get_batch(self, split, batch_size=None, seq_per_img=None):
        ...
        for i in range(batch_size):
            # fetch image
            tmp_id, tmp_id_isg, tmp_fc, tmp_att, tmp_isg, tmp_ssg, ix, tmp_wrapped = self._prefetch_process[split].get()
            id_bath.append(tmp_id)
            id_bath_isg.append(tmp_id_isg)
    ...
    def __getitem__(self, index):
        ...
        if self.use_att:
            ...
            if self.use_isg:
                path_temp = os.path.join(self.input_isg_dir, str(self.info_isg['images'][ix]['id']) + '.npy')
                if os.path.isfile(path_temp):
                    isg_info = np.load(os.path.join(path_temp),allow_pickle=True)
        ...
        return (
                ...
                self.info_isg['images'][ix]['id'],
                # np.load(os.path.join(self.input_fc_dir, str(self.info_isg['images'][ix]['id']) + '.npy'),allow_pickle=True),
                fc_feat,
                att_feat,
                isg_data,
                ssg_data,
